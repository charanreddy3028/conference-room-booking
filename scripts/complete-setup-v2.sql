-- Complete Supabase setup for Conference Room Booking with Secret Keys
...
-- Create bookings table with booking_secret field
...
CREATE INDEX idx_bookings_secret ON public.bookings(booking_secret);
...
-- Insert sample bookings for today with secret keys
INSERT INTO public.bookings (room_id, room_name, floor, booked_by, booking_secret, date, start_time, end_time, status)
SELECT 
    r.id,
    r.name,
    r.floor,
